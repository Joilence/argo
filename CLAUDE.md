...
- Gemini TTS returns headerless PCM (`audio/L16;codec=pcm;rate=24000`), which `ffmpeg -i pipe:0` cannot sniff. Pass `parseRawAudioMime(mimeType)` as `convertToWav`'s third argument. The rate must come from the response: a wrong one does not error, it pitches and stretches the voice. Little-endian despite RFC 2586 specifying network byte order, matching what Google actually sends.

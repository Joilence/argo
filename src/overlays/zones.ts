...
export async function injectIntoZone(
  ...
  await page.evaluate(([id, html, styles, css, instId]) => {
    // Append to documentElement (<html>), not body, so overlays survive
    // aggressive app re-renders that wipe body children.
...
export async function removeZone(page: Page, zone: Zone, instanceId?: string): Promise<void> {
  ...
  await page.evaluate(([id, instId]) => {
    ...
    // Without this check, a fire-and-forget showOverlay from a previous scene can
    // remove the current scene's overlay when they share the same zone.
